        ...
        .then(data => {
            ...
            if (data.status == "success") {
                ...
                for (count in data.data) {
                    ...
                    <p><br>Date     &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;       : <input type="text" value="${data.data[count].date}" id="date${id}" readOnly><br><br>
                    <p>Merchant  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  :<input type="text" value="${data.data[count].customer}" id="customer${id}" autocomplete="off"readOnly list="mer${id}"><datalist id="mer${id}"></datalist><br>
                    <br>Currency    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;     : <input type="text" value="${data.data[count].currency}" id="currency${id}" list="curr${id}" autocomplete="off"readOnly >
                    <datalist id="curr${id}"></datalist><br>
                   
                   <br>Refno   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;      : <input type="text" value="${data.data[count].refno}" id="refno${id}" readOnly><br>
                   <br>Total Amount &nbsp;: ${data.data[count].symbol}<input type="number" value="${data.data[count].total}" id="total${id}" readOnly><br>
                   ...
                   `;
                    for (item in data.data[count].expense_item) {
                        ...
                        Amount&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: ${data.data[count].symbol}<input type="text" value="${data.data[count].expense_item[item].amount}" id="amount${data.data[count].expense_item[item].id}" readOnly><br><br>
...
function put_expense(id) {
    ...
    for (i in child_items_id) {
        total = parseFloat(total) + parseFloat(document.getElementById(`amount${child_items_id[i]}`).value);
        expense_items.push({
            "id": child_items_id[i],
    ...
    fetch(url).then(response => {
        ...
        .then(data => {
            if (data.status == "success") {
                var ele = ``;
                for (i in data.data) {
                    ele += `<option value="${data.data[i].name}"> ${data.data[i].name}</option>`
                }
